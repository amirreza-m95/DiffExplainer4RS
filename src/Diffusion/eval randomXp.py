...
# Random explainer arguments
parser.add_argument('--random_seed', type=int, default=42, nargs='?')
parser.add_argument('--num_random_runs', type=int, default=5, nargs='?')
parser.add_argument('--eval_method', type=str, default="diffusion", choices=["diffusion", "random", "lxr"], help="Which explainer to evaluate: diffusion, random, or lxr")
...
# ========== RANDOM EXPLANER FUNCTIONS ==========
def find_random_mask(user_tensor, item_tensor, item_id, recommender, **kw_dict):
    """
    Generate random counterfactual explanations by randomly selecting items to remove.
    Returns a dictionary mapping item indices to random scores (1.0 for selected items).
    """
    # Get user history (items that user has interacted with)
    user_history = torch.where(user_tensor > 0)[0].cpu().numpy()
    
    # Remove the recommended item from consideration
    user_history = user_history[user_history != item_id]
    
    # Create random explanation scores
    item_sim_dict = {}
    for item_idx in user_history:
        # Assign random scores between 0 and 1
        item_sim_dict[item_idx] = np.random.random()
    
    return item_sim_dict

def single_user_expl_random(user_vector, user_tensor, item_id, item_tensor, recommender, **kw_dict):
    """
    Generate random explanations for a single user.
    Returns a dictionary of explanations, sorted by their random scores.
    """
    user_hist_size = int(np.sum(user_vector))
    
    # Get random-based explanations
    sim_items = find_random_mask(user_tensor, item_tensor, item_id, recommender, **kw_dict)
...
def single_user_metrics_random(user_vector, user_tensor, item_id, item_tensor, num_of_bins, recommender_model, expl_dict, **kw_dict):
    """
    Calculate metrics for a single user based on random explanations.
...
# ========== LXR EXPLAINER FUNCTIONS ==========
class LXRExplainer(nn.Module):
    def __init__(self, user_size, item_size, hidden_size):
        super().__init__()
        self.users_fc = nn.Linear(user_size, hidden_size)
        self.items_fc = nn.Linear(item_size, hidden_size)
        self.bottleneck = nn.Sequential(
            nn.Tanh(),
            nn.Linear(hidden_size * 2, hidden_size),
            nn.Tanh(),
            nn.Linear(hidden_size, user_size),
            nn.Sigmoid()
        )
...
def load_lxr_explainer():
    # Set up checkpoint path based on dataset and recommender
    lxr_ckpt = f"checkpoints/recommenders/LXR_{args.data_name}_{args.recommender_name}_0_12_128_35_7.pt"
    user_size = USER_HISTORY_DIM
    item_size = USER_HISTORY_DIM
    hidden_size = 128
    explainer = LXRExplainer(user_size, item_size, hidden_size).to(DEVICE)
    state_dict = torch.load(lxr_ckpt, map_location=DEVICE)
...
def find_lxr_mask(user_tensor, item_tensor, item_id, explainer):
    with torch.no_grad():
...
def single_user_expl_lxr(user_vector, user_tensor, item_id, item_tensor, explainer, **kw_dict):
    user_hist_size = int(np.sum(user_vector))
    sim_items = find_lxr_mask(user_tensor, item_tensor, item_id, explainer)
    POS_sim_items = list(sorted(sim_items.items(), key=lambda item: item[1], reverse=True))[:user_hist_size]
    return POS_sim_items

def single_user_metrics_lxr(user_vector, user_tensor, item_id, item_tensor, num_of_bins, recommender_model, expl_dict, **kw_dict):
    POS_masked = user_tensor
    NEG_masked = user_tensor
    POS_masked[item_id] = 0
    NEG_masked[item_id] = 0
...
def eval_random_explainer():
    """
    Main evaluation function for the random explainer.
    """
    print(f"=== Evaluating Random Explainer ===")
    print(f"Dataset: {args.data_name}")
    print(f"Recommender: {args.recommender_name}")
    print(f"Random seed: {args.random_seed}")
    print(f"Number of runs per user: {args.num_random_runs}")
    print(f"Device: {DEVICE}")
    ...
    with torch.no_grad():
        for i in range(static_test_array.shape[0]):
            ...
            # Run multiple random explanations for this user
            user_DEL = np.zeros(num_of_bins + 1)
            user_INS = np.zeros(num_of_bins + 1)
            user_NDCG = np.zeros(num_of_bins + 1)
            user_POS_at_1 = np.zeros(num_of_bins + 1)
            user_POS_at_5 = np.zeros(num_of_bins + 1)
            user_POS_at_10 = np.zeros(num_of_bins + 1)
            user_POS_at_20 = np.zeros(num_of_bins + 1)
            user_POS_at_50 = np.zeros(num_of_bins + 1)
            user_POS_at_100 = np.zeros(num_of_bins + 1)
            user_NEG_at_1 = np.zeros(num_of_bins + 1)
            user_NEG_at_5 = np.zeros(num_of_bins + 1)
            user_NEG_at_10 = np.zeros(num_of_bins + 1)
            user_NEG_at_20 = np.zeros(num_of_bins + 1)
            user_NEG_at_50 = np.zeros(num_of_bins + 1)
            user_NEG_at_100 = np.zeros(num_of_bins + 1)
            
            for run in range(args.num_random_runs):
                # Generate random explanations
                user_expl = single_user_expl_random(user_vector, user_tensor, item_id, item_tensor, 
                                                   recommender, **kw_dict)
                
                # Calculate metrics for this run
                res = single_user_metrics_random(user_vector, user_tensor, item_id, item_tensor, 
                                                num_of_bins, recommender, user_expl, **kw_dict)
                
                # Accumulate metrics for this user
                user_DEL += res[0]
                user_INS += res[1]
                user_NDCG += res[2]
                user_POS_at_1 += res[3]
                user_POS_at_5 += res[4]
                user_POS_at_10 += res[5]
                user_POS_at_20 += res[6]
                user_POS_at_50 += res[7]
                user_POS_at_100 += res[8]
                user_NEG_at_1 += res[9]
                user_NEG_at_5 += res[10]
                user_NEG_at_10 += res[11]
                user_NEG_at_20 += res[12]
                user_NEG_at_50 += res[13]
                user_NEG_at_100 += res[14]
            
            # Average metrics across runs for this user
            users_DEL += user_DEL / args.num_random_runs
            users_INS += user_INS / args.num_random_runs
            NDCG += user_NDCG / args.num_random_runs
            POS_at_1 += user_POS_at_1 / args.num_random_runs
            POS_at_5 += user_POS_at_5 / args.num_random_runs
            POS_at_10 += user_POS_at_10 / args.num_random_runs
            POS_at_20 += user_POS_at_20 / args.num_random_runs
            POS_at_50 += user_POS_at_50 / args.num_random_runs
            POS_at_100 += user_POS_at_100 / args.num_random_runs
            NEG_at_1 += user_NEG_at_1 / args.num_random_runs
            NEG_at_5 += user_NEG_at_5 / args.num_random_runs
            NEG_at_10 += user_NEG_at_10 / args.num_random_runs
            NEG_at_20 += user_NEG_at_20 / args.num_random_runs
            NEG_at_50 += user_NEG_at_50 / args.num_random_runs
            NEG_at_100 += user_NEG_at_100 / args.num_random_runs
    
    # Calculate averages
    num_users = static_test_array.shape[0]
    print(f"\n=== Random Explainer Evaluation Results ===")
    print(f"Number of users evaluated: {num_users}")
    print(f"Number of runs per user: {args.num_random_runs}")
    print(f"Random seed: {args.random_seed}")
    print(f"Dataset: {args.data_name}, Recommender: {args.recommender_name}")
...
def eval_lxr_explainer():
    """
    Main evaluation function for the LXR explainer.
    """
    print(f"=== Evaluating LXR Explainer ===")
    print(f"Dataset: {args.data_name}")
    print(f"Recommender: {args.recommender_name}")
    print(f"Device: {DEVICE}")
    ...
    # Load LXR explainer
    explainer = load_lxr_explainer()
    print(f"Loaded LXR explainer")
    ...
    with torch.no_grad():
        for i in range(static_test_array.shape[0]):
            ...
            # Generate explanations using LXR explainer
            user_expl = single_user_expl_lxr(user_vector, user_tensor, item_id, item_tensor, explainer, **kw_dict)
            
            # Calculate metrics
            res = single_user_metrics_lxr(user_vector, user_tensor, item_id, item_tensor, 
                                         num_of_bins, recommender, user_expl, **kw_dict)
    ...
    # Calculate averages
    num_users = static_test_array.shape[0]
    print(f"\n=== LXR Explainer Evaluation Results ===")
    print(f"Number of users evaluated: {num_users}")
    print(f"Dataset: {args.data_name}, Recommender: {args.recommender_name}")
    print(f"\nAverage Metrics:")
    print(f"DEL: {np.mean(users_DEL)/num_users:.4f}")
...
if __name__ == "__main__":
    print("=== Counterfactual Explanation Evaluation ===")
    print("Usage examples:")
    print("  # Evaluate diffusion model")
    print("  python src/Diffusion/eval.py")
    print("")
    print("  # Evaluate random explainer")
    print("  python src/Diffusion/eval.py --eval_method random")
    print("")
    print("  # Evaluate random explainer with custom parameters")
    print("  python src/Diffusion/eval.py --eval_method random --num_random_runs 10 --random_seed 123")
    print("")
    ...
    # Add argument for evaluation method
    import sys
    eval_method = "diffusion"  # default
    if "--eval_method" in sys.argv:
        idx = sys.argv.index("--eval_method")
        if idx + 1 < len(sys.argv):
            eval_method = sys.argv[idx + 1]
    
    if eval_method == "random":
        print("Running Random Explainer Evaluation...")
        results = eval_random_explainer()
    elif eval_method == "lxr":
        print("Running LXR Explainer Evaluation...")
        results = eval_lxr_explainer()
    else:
