...
EPOCHS = 20
LEARNING_RATE = 0.005
LAMBDA_CF = 15.0  # Counterfactual loss weight
LAMBDA_IMPORTANCE = 5.0  # NEW: Importance loss weight
LAMBDA_L1 = 0.9   # Weight for L1 loss
...
def compute_gradient_importance_loss(x0_hat, recommender, top1_indices, x0):
    ...
    for user_idx in range(batch_size):
        ...
        model_importance = x0_hat[user_idx].detach()  # Use original model output
...
# ========== TRAINING LOOP ==========
def loss_visualizer(total_losses, recon_losses, cf_losses, importance_losses, l1_losses, preserve_losses, change_rates):
    """
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Losses Over Epochs (Gradient Importance)')
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig('loss_curves_gradient_importance.png')
    plt.close()
...
def train_denoiser():
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(0, num_samples, BATCH_SIZE):
            ...
            # NEW: Gradient-based importance loss
            importance_loss = compute_gradient_importance_loss(x0_hat, recommender, top1_indices, x0)
            ...
            # Combined loss with importance loss
            loss = masked_recon_loss + LAMBDA_CF * cf_loss + LAMBDA_IMPORTANCE * importance_loss + LAMBDA_L1 * masked_l1_loss + LAMBDA_PRESERVE * preserve_loss
            ...
            total_importance_loss += importance_loss.item() * x0.size(0)
            ...
            if epoch == middle_epoch and i == middle_batch * BATCH_SIZE:
                print(f"\n=== MIDDLE EPOCH {epoch+1} MIDDLE BATCH INSPECTION (GRADIENT IMPORTANCE) ===")
        ...
        avg_importance_loss = total_importance_loss / num_samples
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} - Total Loss: {avg_loss:.4f} | Recon Loss: {avg_recon_loss:.4f} | CF Loss: {avg_cf_loss:.4f} | Importance Loss: {avg_importance_loss:.4f} | L1 Loss: {avg_l1_loss:.4f} | Preserve Loss: {avg_preserve_loss:.4f} | Change Rate: {change_rate:.2%}")
        ...
        if avg_loss < best_loss:
            ...
            checkpoint_filename = f"best_gradient_importance_denoiser_ML1M_bs{BATCH_SIZE}_lr{LEARNING_RATE}_lcf{LAMBDA_CF}_imp{LAMBDA_IMPORTANCE}_l1{LAMBDA_L1}_pres{LAMBDA_PRESERVE}.pt"
    ...
    loss_visualizer(total_losses, recon_losses, cf_losses, importance_losses, l1_losses, preserve_losses, change_rates)
